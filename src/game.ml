...
let set_level (id : int) =
  ...
  (* On réinitialise la liste des blocs, cela permet de décharger les blocs lors d'un changement de niveau *)
  levelBlockList := [];
  level := id;
  ...
  try
    while true; do
      ...
      if (not (String.starts_with ~prefix:"####" line) && String.length line > 2) then begin
        ...
        | "Platform" -> typeBlock := Block_type.Platform
        ...
        | "FlagTop" -> typeBlock := Block_type.FlagTop
        | "Lava" -> typeBlock := Block_type.Lava
        | "FlagBottom" -> typeBlock := Block_type.FlagBottom
...
let update dt =
  if(!canStart) then begin
    ...
    (* C'était pour éviter la dépendance circulaire, au lieu de changer direct, on change à la prochaine frame, donc ici, dans le update *)
